...
# ---------- constants matching verl's preprocessing ----------
# verl's rl_dataset.py adds a system message if not present, then calls
# tokenizer.apply_chat_template(messages, add_generation_prompt=True).
# The logged input was decoded with skip_special_tokens=True, yielding:
    ...
    for fname in files:
        ...
        with open(os.path.join(logs_dir, fname)) as fh:
            for line_idx, line in enumerate(fh):
                ...
                if not inp.startswith(INPUT_PREFIX) or not inp.endswith(INPUT_SUFFIX):
                    skipped += 1
                    continue
                question = inp[len(INPUT_PREFIX):-len(INPUT_SUFFIX)]
                positives.append({
                    "question": question,
